...
/*   Updated: 2024/11/25 09:58:22 by cde-sous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	only_one_philo(t_philo *philo)
{
	if (philo->table->nb_philo == 1)
	{
		...
		is_dead_in_action(philo, WAIT_DEATH);
		pthread_mutex_unlock(philo->fork_left);
		return (0);
	}
	return (1);
...
int	is_dead_in_action(t_philo *philo, t_status action)
{
	...
	// pthread_mutex_lock(&philo->table->start_lock); // testing
	current_time = get_time_relative(philo->table);
	// pthread_mutex_unlock(&philo->table->start_lock); // testing
	if (action == EAT || action == WAIT)
		action_time = philo->table->time_eat;
	else if (action == SLEEP)
		action_time = philo->table->time_sleep;
	else if (action == WAIT_DEATH)
		action_time = philo->table->time_death;
...
int	actions(t_philo *philo)
{
	if (philo->times_eaten == philo->table->max_meals)
		return (0);
	if (check_death_status(philo->table) == 0)
		// not sure if useful since checked right before printing first fork
		return (0);
...
void	*run_routine(void *arg)
{
	...
	if (philo->id % 2 == 0)
		if (is_dead_in_action(philo, WAIT) == 0)
			return (0);
	while (1)
		if (actions(philo) == 0)
			break ;
